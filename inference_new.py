...
# These thresholds came out of fiddling with the DeepPCB validation set.
# Worth re-checking precision/recall before changing them.
WINDOW_SIZE = 128
STRIDE = WINDOW_SIZE // 4               # 75% overlap catches defects that straddle window edges
SIMILARITY_THRESHOLD = 0.95             # SSIM below this = patch is suspect; biased toward over-reporting
CLASSIFIER_CONFIDENCE_THRESHOLD = 0.80  # most real defects classify > 0.9; this trims the noise
NMS_IOU_THRESHOLD = 0.2                 # tight: PCB defects are small, don't want one defect counted twice
BATCH_SIZE = 32                         # fits an 8 GB GPU; CPU-bound runs are fine too
...
class PCBDefectPipeline:
    ...
    def __init__(self, model_path=MODEL_PATH, golden_dir=GOLDEN_DIR):
    ...
    def _build_golden_database(self):
    ...
    def find_best_match(self, input_image):
    ...
    def _classify_batch(self, patches):
        ...
        return list(zip(idxs.tolist(), confs.tolist()))

    def detect_anomalies(self, input_image, golden_image):
        ...
        for start in range(0, len(patches), BATCH_SIZE):
            ...
            for (idx, conf), box in zip(results, boxes[start:end]):
    ...
    def draw_detections(image, detections):
        ...
        colors = {
            label: tuple(int(c * 255) for c in cmap(i / max(len(labels), 1))[:3])
        ...
        for d in detections:
            ...
            text_h = tb[3] - tb[1]
            draw.rectangle(
                [x1, y1 - text_h - 5, x1 + text_w + 10, y1], fill=color
            )
